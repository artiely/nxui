    ...
    <div class="md-picker-column-container">
            ...
            <ul class="column-list" :style="{ 'padding-top': `${style.maskerHeight}px` }">
              <template v-for="(item, j) in colunm">
                <li class="column-item" :class="{
                  ...
                  }" :key="j" v-text="item.text || item.label">
                </li>
              </template>
            </ul>
          </div>
      ...
      <div class="md-picker-column-hooks">
        <template v-for="n in cols">
          <div class="md-picker-column-hook" :key="n - 1" @touchstart="$_onColumnTouchStart($event, n - 1)" @mousedown="$_onColumnTouchStart($event, n - 1, true)" @touchmove="$_onColumnTouchMove($event, n - 1)" @mousemove="$_onColumnTouchMove($event, n - 1, true)" @touchend="$_onColumnTouchEnd($event, n - 1)" @mouseup="$_onColumnTouchEnd($event, n - 1, true)"></div>
        </template>
      </div>
    </div>
  </div>
    ...
    $_initColumnIndex() {
      ...
      this.$_getColumnIndexByDefault(
        ...
        (columnIndex, itemIndex) => {
          ...
          if (!scroller) {
            warn(
              `initialColumnIndex: scroller of column ${columnIndex} is undefined`
            )
            return 1
          }
...
@picker-color-active:#000;
@picker-font-weight-active: 500;
